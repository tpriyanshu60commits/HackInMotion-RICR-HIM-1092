...
const getAqiStyle = (aqi) => {
  if (aqi <= 50)
    return {
      ...
      bg: 'bg-green-500/10',
      glow: 'shadow-[inset_0_0_20px_rgba(34,197,94,0.1)]',
    };
  if (aqi <= 100)
    return {
...
const LocationCard = ({ loc }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-[20px] ${aqiStyle.glow}`}
      ></div>
      ...
      <div className="flex flex-col md:items-center min-w-[120px]">
        ...
        <div className="flex items-baseline gap-2">
          ...
          <span
            className={`text-sm font-bold px-2 py-0.5 rounded-md ${aqiStyle.bg} ${aqiStyle.color}`}
          >
            {loc.status}
          </span>
        </div>
      ...
      <div className="hidden md:flex shrink-0 ml-2">
        <ChevronRight
          size={24}
          className="text-gray-500 opacity-50 group-hover:opacity-100 group-hover:translate-x-1.5 transition-all duration-300 ease-in-out"
        />
      </div>
    </motion.div>
  );
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      if (query.length > 2 && !selectedResult) {
        setIsSearching(true);
        try {
          const res = await environmentService.searchGeocoding(query);
          const data = res.data;
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 pt-4">
        <div>
          ...
          <p className="text-sm text-gray-400 font-medium">
            Monitor environmental conditions across your saved locations.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
          <div className="relative w-full sm:w-64 group">
            <Search
              className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-green-400 transition-colors"
              size={18}
            />
            <input
              type="text"
      ...
      {loading && locations.length === 0 ? (
        <div className="flex flex-col gap-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="w-full rounded-[20px] bg-white/[0.04] p-5 md:p-6 flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-6"
            >
