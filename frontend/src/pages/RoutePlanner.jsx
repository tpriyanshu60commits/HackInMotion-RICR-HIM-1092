    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">Estimate environmental exposure along your journey</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-6">
          <form onSubmit={calculateRisk} className="glass p-6 rounded-3xl space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1.5 text-muted-foreground">Start Location</label>
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5 text-muted-foreground">Destination</label>
              <div className="relative">
          ...
          {risk && (
            <div className={`glass p-6 rounded-3xl border-2 ${risk.level === 'High' ? 'border-destructive/50 bg-destructive/5' : risk.level === 'Moderate' ? 'border-warning/50 bg-warning/5' : 'border-success/50 bg-success/5'}`}>
              <div className="flex items-center gap-3 mb-4">
                <ShieldAlert className={`w-8 h-8 ${risk.level === 'High' ? 'text-destructive' : risk.level === 'Moderate' ? 'text-warning' : 'text-success'}`} />
                <div>
        ...
        <div className="lg:col-span-2 glass rounded-3xl overflow-hidden min-h-[400px] border border-border flex items-center justify-center bg-muted/20 relative">
          ...
          <div className="text-center z-10 bg-card p-6 rounded-2xl shadow-xl border border-border max-w-sm mx-4">
            ...
            <p className="text-sm text-muted-foreground">Map rendering is active. Enter locations to visualize your route and real-time environmental hazards.</p>
          </div>
        </div>
      </div>
    </div>
