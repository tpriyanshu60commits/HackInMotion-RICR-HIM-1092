...
export const Alerts = () => {
  ...
  const getAlertStyles = (type) => {
    switch(type) {
      case 'high-risk':
        return { icon: ShieldAlert, color: 'text-red-400', bg: 'bg-red-500/15', border: 'border-red-500/20', shadow: 'shadow-[0_0_18px_rgba(239,68,68,0.12)]', label: 'HIGH RISK' };
      case 'moderate-risk':
        return { icon: ArrowUpRight, color: 'text-amber-400', bg: 'bg-amber-500/15', border: 'border-amber-500/20', shadow: 'shadow-[0_0_18px_rgba(245,158,11,0.12)]', label: 'MODERATE' };
      case 'forecast':
        return { icon: ArrowUpRight, color: 'text-blue-400', bg: 'bg-blue-500/15', border: 'border-blue-500/20', shadow: 'shadow-[0_0_18px_rgba(59,130,246,0.12)]', label: 'FORECAST' };
      case 'improvement':
        return { icon: ArrowDownRight, color: 'text-green-400', bg: 'bg-green-500/15', border: 'border-green-500/20', shadow: 'shadow-[0_0_18px_rgba(34,197,94,0.12)]', label: 'IMPROVED' };
      default:
        return { icon: ShieldAlert, color: 'text-gray-400', bg: 'bg-gray-500/15', border: 'border-gray-500/20', shadow: 'shadow-[0_0_18px_rgba(156,163,175,0.12)]', label: 'ALERT' };
    }
  };
            ...
            alerts.map(alert => {
              ...
              return (
                ...
                >
                  ...
                  {/* Left: Icon */}
                  <div className={`shrink-0 w-14 h-14 rounded-2xl ${styles.bg} border ${styles.border} ${styles.shadow} flex items-center justify-center group-hover:scale-105 transition-transform duration-300`}>
                  ...
                  <div className="flex-1 flex flex-col justify-center min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 text-xs md:text-sm text-gray-400">
                      ...
                      <span className="truncate">{alert.location?.city || alert.location?.name || 'Unknown Location'}</span>
                    </div>
                  </div>
                  ...
                  <div className="md:text-right shrink-0 flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center mt-2 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-white/[0.05]">
                    <span className={`text-sm font-bold tracking-wide uppercase mb-0 md:mb-1.5 ${styles.color}`}>
                      {styles.label}
                    </span>
