...
export default function CityComparison() {
  ...
  const removeCity = (cityName) => {
    ...
    setCityData(prev => prev.filter(c => c.name !== cityName));
  };

  const filteredCities = CITIES.filter(c => c.name.toLowerCase().includes(search.toLowerCase()) && !selectedCities.find(sc => sc.name === c.name));
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">Compare environmental risk across multiple locations</p>
        </div>
      </div>
      ...
      <div className="glass p-6 rounded-3xl">
          ...
          {search && (
            <div className="absolute top-full left-0 mt-2 w-full max-w-md bg-card border border-border rounded-xl shadow-lg z-10 max-h-60 overflow-y-auto">
              ...
              {filteredCities.length === 0 && (
                <div className="p-4 text-muted-foreground text-center">No cities found</div>
              )}
            </div>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-3 mb-8">
          {selectedCities.map(city => (
            <div key={city.name} className="flex items-center gap-2 bg-primary/10 text-primary px-4 py-2 rounded-full border border-primary/20 font-medium shadow-sm">
              {city.name}
              <button onClick={() => removeCity(city.name)} className="hover:bg-primary/20 rounded-full p-0.5 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
          ))}
        ...
        {cityData.length > 0 ? (
          <div className="h-96 w-full mt-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={cityData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: 'var(--card)', borderColor: 'var(--border)', borderRadius: '12px', boxShadow: 'var(--shadow)' }}
          ...
          <div className="text-center py-20 border-2 border-dashed border-border rounded-2xl">
            <p className="text-muted-foreground">Add cities to begin comparison</p>
          </div>
        )}
      </div>
    </div>
