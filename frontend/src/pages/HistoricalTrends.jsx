...
export const HistoricalTrends = () => {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      try {
        ...
        if (locationId) {
          const apiRange = range.replace(' Days', 'd');
          const response = await environmentService.getSnapshots(locationId, apiRange);
          if (response.data && response.data.success && response.data.data.length > 0) {
            fetchedData = response.data.data.map((d) => ({
              ...d,
        ...
        if (!fetchedData || fetchedData.length === 0) {
          const historyResponse = await environmentService.getHistory(
            selectedLocation.lat,
            selectedLocation.lng,
            days
          );
          if (historyResponse && historyResponse.success) {
            fetchedData = historyResponse.data;
          }
        }
        ...
        if (fetchedData && fetchedData.length > 0) {
          ...
          setError('No historical data available for this location.');
        }
      } catch (err) {
        console.error(err);
  ...
  const calculateStats = () => {
    ...
    const firstHalfAvgAqi = firstHalf.length
      ? firstHalf.reduce((acc, curr) => acc + curr.aqi, 0) / firstHalf.length
      : 1;
    const secondHalfAvgAqi = secondHalf.length
      ? secondHalf.reduce((acc, curr) => acc + curr.aqi, 0) / secondHalf.length
      : 1;
    ...
    if (type === 'pm25') {
      ...
      if (value <= 55.4)
        return {
          ...
          bg: 'bg-orange-500',
          shadow: 'shadow-[0_0_8px_rgba(249,115,22,0.8)]',
        };
      return {
        text: 'Unhealthy',
        color: 'text-red-500',
  ...
  const trendColor =
    stats.trend > 0 ? 'text-red-500' : stats.trend < 0 ? 'text-green-500' : 'text-gray-400';
