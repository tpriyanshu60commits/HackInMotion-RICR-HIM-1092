...
async function runAllTests() {
  ...
  const authTests = [
    {
      name: 'Save AI Health Profile',
      method: 'POST',
      path: '/api/ai-health/profile',
      header: normalHeader,
      body: { primaryCity: 'Mumbai', ageGroup: 'adult', conditions: ['Asthma'] },
      expected: 200,
    },
    {
      ...
      header: normalHeader,
      body: { primaryCity: 'Mumbai', ageGroup: 'adult' },
      expected: 201,
    },
    {
      name: 'Get Latest AI Health Report',
      method: 'GET',
  ...
  for (const t of authTests) {
    try {
      ...
      if (res.status === t.expected) {
        ...
        console.error(
          `❌ [FAIL] Auth User: ${t.name} -> Expected ${t.expected}, got ${res.status}`,
          res.data
        );
        failed++;
      }
  ...
  const guestTests = [
    {
      name: 'Guest Save AI Health Profile',
      method: 'POST',
      path: '/api/ai-health/profile',
      header: guestHeader,
      body: { primaryCity: 'Bhopal', ageGroup: 'senior', conditions: ['COPD'] },
      expected: 200,
    },
    {
      name: 'Guest Generate AI Health Report',
      method: 'POST',
      path: '/api/ai-health/report/generate',
      header: guestHeader,
      body: { primaryCity: 'Bhopal' },
      expected: 201,
    },
    {
      name: 'Guest Get Latest AI Health Report',
      method: 'GET',
      path: '/api/ai-health/report/latest',
      header: guestHeader,
      expected: 200,
  ...
  for (const t of guestTests) {
    try {
      ...
      if (res.status === t.expected) {
        ...
        console.error(
          `❌ [FAIL] Guest User: ${t.name} -> Expected ${t.expected}, got ${res.status}`,
          res.data
        );
        failed++;
      }
  ...
  const envTests = [
    {
      name: 'GET /api/environment/city?city=Mumbai',
      method: 'GET',
      path: '/api/environment/city?city=Mumbai',
      header: normalHeader,
      expected: 200,
    },
    {
      name: 'GET /api/environment/city?city=Bhopal',
      method: 'GET',
      path: '/api/environment/city?city=Bhopal',
      header: normalHeader,
      expected: 200,
    },
    {
      name: 'GET /api/environment/compare?cities=Mumbai,Bhopal',
      method: 'GET',
      path: '/api/environment/compare?cities=Mumbai,Bhopal',
      header: normalHeader,
      expected: 200,
    },
    {
      name: 'GET /api/environment/city (missing city -> 400)',
      method: 'GET',
      path: '/api/environment/city',
      header: normalHeader,
      expected: 400,
    },
  ];
  ...
  for (const t of envTests) {
    try {
      ...
      if (res.status === t.expected) {
        ...
        console.error(
          `❌ [FAIL] Environment: ${t.name} -> Expected ${t.expected}, got ${res.status}`,
          res.data
        );
        failed++;
      }
  ...
  const searchTests = [
    {
      name: 'GET /api/locations/search?q=Mumbai',
      method: 'GET',
      path: '/api/locations/search?q=Mumbai',
      header: {},
      expected: 200,
    },
    {
      name: 'GET /api/locations/search?q=Bhopal',
      method: 'GET',
      path: '/api/locations/search?q=Bhopal',
      header: {},
      expected: 200,
    },
    {
      name: 'GET /api/location/search?q=Delhi',
      method: 'GET',
      path: '/api/location/search?q=Delhi',
      header: {},
      expected: 200,
    },
  ];
  ...
  for (const t of searchTests) {
    try {
      ...
      if (res.status === t.expected && Array.isArray(res.data?.data) && res.data.data.length > 0) {
        console.log(
          `✅ [PASS] Location Search: ${t.name} -> Status ${res.status} (${res.data.data.length} results: ${res.data.data[0].display_name})`
        );
        passed++;
      } else {
        console.error(
          `❌ [FAIL] Location Search: ${t.name} -> Expected ${t.expected} with array, got ${res.status}`,
          res.data
        );
        failed++;
      }
