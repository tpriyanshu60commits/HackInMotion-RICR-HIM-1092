...
export const calculateBaseRisk = (aqi) => {
  ...
  if (aqi <= 150)
    return { level: 'UNHEALTHY_SENSITIVE', label: 'Unhealthy for Sensitive Groups', severity: 3 };
...
export const getPersonalizedRisk = (baseRisk, healthProfile) => {
  ...
  const hasRespiratory =
    respiratoryCondition ||
    asthma ||
    diagnosedConditions.includes('Asthma') ||
    diagnosedConditions.includes('Respiratory condition (e.g. COPD)');
  const hasHeart =
    heartCondition ||
    diagnosedConditions.includes('Heart condition') ||
    diagnosedConditions.includes('Hypertension');
  ...
  const isSensitive =
    hasRespiratory || hasHeart || hasChildren || hasElderly || customIssue.trim().length > 0;

  if (isSensitive && baseRisk.severity >= 2) {
    personalizedSeverity = Math.min(6, baseRisk.severity + 1);

    if (hasRespiratory)
      reasons.push('Respiratory conditions increase susceptibility to poor air quality.');
    ...
    if (customIssue.trim().length > 0)
      reasons.push(
        `We noted your custom condition: "${customIssue}". Please take extra precautions.`
      );
  }
  ...
  return {
    ...
    explanation:
      reasons.length > 0
        ? reasons.join(' ')
        : 'Your profile does not indicate heightened sensitivity.',
  ...
  switch (severity) {
    ...
    case 4:
      return [
        ...
        'Keep windows closed and run air purifiers.',
      ];
    case 5:
      return [
    ...
    case 6:
      return [
        ...
        'Remain indoors.',
        'Keep all windows closed and maximize air purification.',
      ];
    default:
      return [];
  }
